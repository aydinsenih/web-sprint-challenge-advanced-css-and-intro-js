...
function get20s(arr){
  // let newArr = [];
  // for(let i of arr){
  //   let years = i.years.split(" - ");
  //   if(years[0] >= 1900 && years[1] < 2000){
  //     newArr.push(i.name);
  //   }
  // }
  // return newArr;
  return arr.filter(i => i.years.split(" - ")[0] >= 1900 && i.years.split(" - ")[1] < 2000).map(k => k.name);
}
...
function getHTML(arr){
  // for(let i in arr){
  //   document.getElementById("artists-container").innerHTML = `<div id="artist">
  //     <div class="image">
  //         <img src="https://images.fineartamerica.com/images/artworkimages/mediumlarge/3/starry-night-by-vincent-van-gogh-vincent-van-gogh.jpg"/>
  //     </div>
  //     <div class = "name">
  //       <a href="${arr[i].wikipedia}"> ${arr[i].name}h</a>
  //     </div>
...
// console.log(randomize(artists));
 ...
 let arr = [...artists];
 console.log(arr.filter(i => i.years.split(" - ")[0] >= 1900 && i.years.split(" - ")[1] < 2000).map(k => k.name));
